...
solutions for exposing a knowledge graph that was derived from a OMERO instance
as a public web resource. We measured the query response time (QRT) of three
...
number of triples from the virtual knowledge graph in the Ontop endpoint and
uploading to Fuseki. We only studied Fuseki in this exercise. All 10 queries
...
all complete in about 1 second. We assume this limit comes from the network
bandwith and other factors not related to the nature of the query or the
endpoint implementation. As the graphs grow bigger, the query response time
first curve first remains flat for a given query. At a certain size, the curve
...
I view of the the observed linear scaling of query response time with number of
triples in the queried graph, we see a strong demand for query opt
